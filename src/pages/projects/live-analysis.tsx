...
function eventToMessage(event: Record<string, any>): string {
  ...
  if (event.step === "error")
    return `Pipeline error: ${event.msg ?? event.detail ?? "Unknown error"}`;
  if (event.step === "timeout")
    return `Pipeline timed out. ${event.msg ?? "Click Retry to continue."}`;
  const parts: string[] = [];
...
function severityRowClass(severity: LogSeverity) {
  switch (severity) {
    ...
    case "error":
      return "bg-destructive/5 border-l-2 border-destructive/50";
    case "success":
...
export function LiveAnalysisPage() {
  ...
  >("connecting");
  const [pipelineStatus, setPipelineStatus] = useState<"running" | "done" | "error" | "timeout">(
    "running",
  );
          ...
          await fetchEventSource(`${API_BASE}/projects/${id}/stream`, {
            ...
            onmessage: (ev) => {
              try {
                ...
                setLogs((prev) => {
                  if (data.step === "done" && prev.some((l) => l.severity === "success"))
                    return prev;
                  return [
                    ...prev,
                    {
  ...
  return (
    <div
      className="space-y-5 max-w-5xl mx-auto flex flex-col"
            ...
            <div className="divide-y divide-border/20">
              {logs.map((log) => (
                ...
                >
                  ...
                  <span
                    className={cn("break-all leading-relaxed", severityTextClass(log.severity))}
                  >
                    {log.message}
                  </span>
                </div>
