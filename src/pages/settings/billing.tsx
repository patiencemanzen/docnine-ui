...
function ChangePlanModal({
  ...
  const [resultMsg, setResultMsg] = useState<{
    type: "success" | "redirect" | "scheduled";
    text: string;
    link?: string;
  } | null>(null);
    ...
    try {
      ...
      if (result.type === "upgrade") {
        if (result.paymentLink) {
          ...
          setResultMsg({
            type: "success",
            text: "Upgraded successfully! Your new plan is now active.",
          });
          setTimeout(() => {
            setResultMsg(null);
            onClose();
        ...
        setResultMsg({
          type: "success",
          text: "Plan upgraded successfully! No additional charge needed.",
        });
        setTimeout(() => {
          setResultMsg(null);
          onClose();
        ...
        await onRefresh();
        const date = result.effectiveAt
          ? format(new Date(result.effectiveAt), "MMM d, yyyy")
          : "next renewal";
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {displayPlans.map((plan: BillingPlan) => {
              ...
              return (
                ...
                >
                  {isCurrent && (
                    <span className="absolute top-2 right-2 text-[9px] font-bold tracking-wide text-muted-foreground uppercase">
                      Current
                    </span>
                  )}
        ...
        <div className="flex gap-2 pt-1">
          <Button variant="outline" className="flex-1" onClick={onClose} disabled={!!resultMsg}>
            Cancel
          </Button>
          <Button
            className="flex-1"
...
function CurrentPlanCard({
  ...
  const daysLeft = sub.trialEndsAt ? differenceInDays(new Date(sub.trialEndsAt), new Date()) : null;
        ...
        <CardHeader>
          ...
          {sub.status === "trialing" && daysLeft !== null && (
            <div className="flex items-center gap-2 rounded-lg border border-primary/20 bg-primary/10 px-3 py-2 text-sm text-primary mt-2">
              ...
              Your trial ends in <strong>{daysLeft > 0 ? `${daysLeft} days` : "today"}</strong>.
            </div>
          )}
          {sub.cancelAtPeriodEnd && sub.currentPeriodEnd && (
            <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm text-muted-foreground mt-2">
              ...
              Cancels on {format(new Date(sub.currentPeriodEnd), "MMMM d, yyyy")}
            </div>
          )}
        </CardHeader>
        ...
        <CardContent className="space-y-5">
          <div className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
            ...
            <div>
              ...
              <p className="font-medium">
                ...
                {sub.extraSeats > 0 && (
                  <span className="text-muted-foreground ml-1">(+{sub.extraSeats} extra)</span>
                )}
              </p>
            </div>
          </div>
          ...
          {sub.pendingPlan && (
            <div className="rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm text-muted-foreground">
              ...
              <strong className="text-foreground capitalize">{sub.pendingPlan}</strong> at next
              renewal.
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            {sub.plan !== "free" && !sub.cancelAtPeriodEnd && sub.status !== "cancelled" && (
              <>
                <Button
                  variant="outline"
                  size="sm"
      ...
      <Dialog open={cancelOpen} onOpenChange={(v) => !v && setCancelOpen(false)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Your plan stays active until the end of the current period. You can resubscribe any
              time.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <div className="flex gap-2 pt-1">
              ...
              >
                {loading === "cancel" ? <Loader1 className="mr-2 h-4 w-4 " /> : null}
                Confirm cancel
              </Button>
            </div>
          </div>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1" disabled={loading === "pause"} onClick={handlePause}>
                {loading === "pause" ? <Loader1 className="mr-2 h-4 w-4 " /> : null}
...
function SeatsCard({ sub, onRefresh }: { sub: SubscriptionData; onRefresh: () => void }) {
    ...
    <Card className="shadow-none">
      ...
      <CardContent>
        <div className="flex items-center gap-3">
          ...
          <span className="text-lg font-semibold w-8 text-center tabular-nums">{qty}</span>
          <Button
            variant="outline"
            size="icon"
            className="h-8 w-8"
...
function PaymentMethodsCard({ onRefresh }: { onRefresh: () => void }) {
  ...
  async function handleSetDefault(id: string) {
    try {
      ...
      setMethods((m) => m.map((pm) => ({ ...pm, isDefault: pm._id === id })));
      await onRefresh();
    } catch (err: any) {
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  >
                    <MethodIcon
                      className={cn(
                        "h-4 w-4",
                        pm.isDefault ? "text-primary" : "text-muted-foreground",
                      )}
                    />
                  </div>
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {pm.isDefault && (
                        <Badge
                          variant="outline"
                          className="text-[10px] px-1.5 py-0 h-4 text-primary border-primary/30 bg-primary/5 shrink-0"
                        >
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            Add company name or VAT number to {invoice?.invoiceNumber ?? "invoice"}.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          <div>
                ...
                <tbody>
                  {invoices.map((inv) => (
                    ...
                    >
                      ...
                      <td className="py-3 font-medium">{fmtMoney(inv.amount, inv.currency)}</td>
                      <td className="py-3">
...
export function BillingPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 space-y-6">
        ...
        <div className="flex items-center gap-3 rounded-lg border border-green-500/20 bg-green-500/10 px-4 py-3 text-sm text-green-400">
          ...
          <div>
            ...
            <p className="mt-0.5 text-green-400/80">
              Your 7-day trial has started. Enjoy all {subscription.planName} features.
            </p>
          </div>
        </div>
      )}
      ...
      {subscription ? (
        <>
          <CurrentPlanCard sub={subscription} usage={usage} onRefresh={handleRefresh} />
    ...
    (async () => {
      ...
      try {
        ...
        setVerifyState({
          ...
          message:
            err?.message ??
            "Payment verification failed. Contact support if your account wasn't updated.",
        });
      }
    })();
  }, []);
