    ...
    <Card className="flex flex-col hover:border-primary/40 transition-colors group shadow-none bg-background">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-2">
          ...
          {security?.grade && (
            <span
              className={`shrink-0 text-xl font-bold tabular-nums ${gradeColour(security.grade)}`}
        ...
        {dominantStatus && <DocStatusBadge status={dominantStatus} compact />}
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            {(["CRITICAL", "HIGH", "MEDIUM", "LOW"] as const).map((sev) => {
              ...
              return (
                <span
                  key={sev}
                  className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${colours[sev]}`}
                >
                  {n} {sev[0]}
                </span>
              );
...
export function DocumentationsPage() {
  ...
  const filteredCompleted = completedProjects
    .filter((p) => {
            ...
            <div className="space-y-2">
              {inProgressProjects.map((p) => {
                ...
                return (
                  <div
                    key={p._id}
                    className="flex flex-wrap items-center justify-between bg-card border border-border rounded-lg px-4 py-3 gap-x-4 gap-y-2"
                  >
