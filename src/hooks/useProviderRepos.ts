...
export function useProviderRepos() {
  ...
  const loadRepos = useCallback(
    async (provider: ProviderKey, page: number, org?: string | null) => {
      ...
      try {
        ...
        if (
          ...
          setApiError(
            `Failed to load ${provider} repositories. ${err?.message || "Please try again."}`,
          );
        }
      } finally {
  ...
  const getFilteredRepos = useCallback(() => {
    ...
    return repos.filter((r) => r.full_name.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [reposState]);
