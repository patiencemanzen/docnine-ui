...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  GITLAB_NOT_CONNECTED: "GitLab account is not connected. Please connect GitLab in Settings first.",
  GITHUB_NOT_CONNECTED: "GitHub account is not connected. Please connect GitHub in Settings first.",
  ...
  PROJECT_RUNNING: "This project is already processing. Please wait for it to complete.",
  PROJECT_ARCHIVED: "This project is archived. Cannot perform this action.",
  TOKEN_EXPIRED: "Your authentication token has expired. Please reconnect your provider.",
...
export class ProjectCreationService {
  static formatErrorMessage(err: unknown): string {
    if (err instanceof ApiException) {
      return ERROR_MESSAGES[err.code] || err.message || "Failed to create project.";
    }
  ...
  static async fromManualUrl(values: ManualProjectFormValues): Promise<ProjectCreationResult> {
    try {
  ...
  static async fromProvider(repo: NormalizedRepo): Promise<ProjectCreationResult> {
    try {
      const repoUrl = repo.html_url || repo.web_url || "";

      if (!repoUrl) {
        throw new Error("Unable to determine repository URL. Please try again or contact support.");
      }
  ...
  static async fromScratch(values: FromScratchFormValues): Promise<ProjectCreationResult> {
    try {
  ...
  static async validateZip(zipFile: File): Promise<{ valid: boolean; message?: string }> {
    try {
