...
export const SESSION_NOISE_ACTIONS = new Set(["AUTH_LOGIN", "AUTH_LOGOUT", "AUTH_SIGNUP"]);
...
export function formatActivityLabel(
  ...
  const actor = isSelf ? "You" : log.actorName?.trim() || log.actorEmail?.trim() || "A teammate";
...
export function visibleMetadata(metadata?: Record<string, unknown> | null) {
  ...
  return Object.fromEntries(
    Object.entries(metadata).filter(([k, v]) => v != null && v !== "" && !HIDDEN_META_KEYS.has(k)),
  );
}
