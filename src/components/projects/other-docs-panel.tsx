        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                >
                  <ChevronDown
                    className={cn("h-3 w-3 transition-transform", showMeta && "rotate-180")}
                  />
          ...
          {showMeta && (
            <p className="text-xs text-muted-foreground mt-1">
              Uploaded by{" "}
              <span className="text-foreground font-medium">{attachment.uploaderName}</span>
            </p>
          )}
...
export function OtherDocsPanel({ projectId }: OtherDocsPanelProps) {
  ...
  const [uploading, setUploading] = useState<
    { file: File; progress: "pending" | "done" | "error" }[]
  >([]);
  ...
  async function handleFiles(files: File[]) {
    ...
    const valid = files.filter((f) => {
      if (f.size > MAX) {
        setError(
          `"${f.name}" exceeds the ${maxFileSizeMb} MB limit and was skipped. Upgrade your plan for a higher file size limit.`,
        );
        return false;
      }
      return true;
  ...
  function handleDescriptionSaved(id: string, desc: string) {
    setAttachments((prev) => prev.map((a) => (a._id === id ? { ...a, description: desc } : a)));
  }
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <label
          ...
          onClick={
            maxAttachments !== null && attachments.length >= maxAttachments
              ? (e) => {
                  e.preventDefault();
                  requirePlan(
                    "More Attachments",
      ...
      {loading && (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-16 rounded-lg border border-border bg-muted/20 animate-pulse"
            />
          ))}
        </div>
      )}
