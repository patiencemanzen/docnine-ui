...
function SchemaDisplay({ schema, depth = 0 }: SchemaProps) {
  ...
  return (
    <span className={cn("font-mono text-[11px]", colorMap[type ?? ""] ?? "text-muted-foreground")}>
      {type ?? "any"}
      {schema.format != null ? (
        <span className="text-muted-foreground/60">({String(schema.format)})</span>
      ) : null}
      {schema.enum != null ? (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <code className="font-mono text-base font-semibold text-foreground break-all">
              {endpoint.path}
            </code>
            {endpoint.deprecated && (
              <Badge
                variant="outline"
                className="border-primary/30 text-primary dark:text-primary text-[10px]"
              >
          ...
          {endpoint.summary && <p className="mt-1 text-sm text-foreground">{endpoint.summary}</p>}
      ...
      <div className="rounded-lg border border-dashed border-border bg-muted/20 p-3">
        <div className="flex items-center gap-1.5 mb-1">
          ...
          {canEdit && !noteEditing && (
            <button
              onClick={() => {
                setNoteDraft(endpoint.customNote ?? "");
                setNoteEditing(true);
              }}
          ...
          <p className="text-xs text-muted-foreground min-h-5">
            {endpoint.customNote || (
              <span className="italic opacity-50">
                No note yet. {canEdit ? "Click edit to add one." : ""}
              </span>
            )}
          </p>
        )}
          ...
          <div className="space-y-3">
            {Object.entries(endpoint.requestBody.content).map(([ct, mc]) => (
              <div key={ct} className="rounded-lg border border-border overflow-hidden">
                <div className="border-b border-border bg-muted/30 px-3 py-1.5 font-mono text-[11px] text-muted-foreground">
                  {ct}
                </div>
                <div className="p-3">
                  ...
                  {mc.example !== null && mc.example !== undefined && (
                    <div className="mt-2">
                      ...
                      <pre className="rounded bg-muted/40 p-2 font-mono text-[11px] overflow-x-auto whitespace-pre-wrap">
                        {typeof mc.example === "string"
                          ? mc.example
                          : JSON.stringify(mc.example, null, 2)}
                      </pre>
                    </div>
                  )}
                </div>
            ...
            {Object.entries(endpoint.responses).map(([code, r]) => (
              <div key={code} className="rounded-lg border border-border overflow-hidden">
                ...
                >
                  <span
                    className={cn(
                      "rounded border px-1.5 py-0.5 text-[11px] font-bold font-mono shrink-0",
                      responseCodeColor(code),
                    )}
                  >
                    {code}
                  </span>
      ...
      <div className="shrink-0 flex items-center gap-2 border-b border-border px-4 py-2.5 bg-muted/10 flex-wrap">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="text-xs text-muted-foreground shrink-0">
            {specState.endpoints.length} endpoints
          </span>
        </div>
