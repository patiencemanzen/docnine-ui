  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex flex-col">
      <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-card shrink-0">
        <div>
          <h2 className="text-[14px] font-semibold">Compare versions : {sectionLabel}</h2>
          <p className="text-[12px] text-muted-foreground mt-0.5">
            Your edit (left) vs the new AI version (right)
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button size="sm" className="h-8" disabled={accepting} onClick={onAcceptAI}>
            {accepting && <Loader1 className="mr-1.5 h-3.5 w-3.5" />}
            Use AI version
          </Button>
          <Button size="sm" variant="outline" className="h-8 gap-1.5" onClick={onClose}>
