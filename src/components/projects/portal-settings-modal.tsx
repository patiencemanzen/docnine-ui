...
import {
  ...
  TabId,
  VISIBILITY_OPTIONS,
} from "@/configs/PortalConfig";
import {
  DEFAULT_PORTAL_TEMPLATE_ID,
  PORTAL_AUDIENCE_LABELS,
  PORTAL_TEMPLATES,
  getPortalTemplate,
} from "@/configs/PortalTemplates";
import {
  ApiPortal,
  PortalAccessMode,
  PortalBranding,
...
export function PortalSettingsModal({
  ...
  const [draftTemplateId, setDraftTemplateId] = useState<PortalTemplateId>(
    DEFAULT_PORTAL_TEMPLATE_ID,
  );
  ...
  useEffect(() => {
    ...
    if (initialPortal !== undefined) {
      ...
      if (initialPortal) {
        ...
        setDraftSeo({
          seoTitle: initialPortal.seoTitle ?? "",
          seoDescription: initialPortal.seoDescription ?? "",
        });
            ...
            <div className="px-6 py-5 space-y-5 overflow-y-auto max-h-[55vh]">
                ...
                <div className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        {
                          value: "public" as PortalAccessMode,
                          label: "Fully Public",
                          desc: "Anyone with the URL can read",
                          icon: Globe,
                        },
                        {
                          value: "password" as PortalAccessMode,
                          label: "Password Protected",
                          desc: "Visitors must enter a password",
                          icon: Lock,
                        },
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setDraftSeo((s) => ({ ...s, seoDescription: e.target.value }))
                      }
                    />
                  </div>
                </div>
              ...
              {activeTab === "sections" && (
                <div className="space-y-3">
                  <p className="text-xs text-muted-foreground">
                    Control which documentation sections appear in the public portal and how they
                    are displayed.
                  </p>
                  {getAllPublishableSections().map(({ key, label, isCustom }) => {
                    ...
                    return (
                      <div
                        key={key}
                        className="flex items-center justify-between rounded-lg border border-border p-3 gap-4"
                      >
              ...
              {activeTab === "templates" && (
                <div className="space-y-4">
                  <p className="text-xs text-muted-foreground">Choose a structure for pages.</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {PORTAL_TEMPLATES.map((template) => {
                      const selected = draftTemplateId === template.id;
                      const isTop =
                        template.layout === "top-nav" || template.layout === "hero-docs";
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            {isTop ? (
                              ...
                              <div className="flex h-16">
                                <div
                                  className={cn("shrink-0 border-r", isDense ? "w-12" : "w-10")}
                                  style={{
                          ...
                          <div className="flex items-start justify-between gap-2">
                            ...
                            {selected && <Check className="h-4 w-4 shrink-0 text-primary mt-0.5" />}
                          </div>
                        </button>
                      );
                    })}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setDraftBranding((b) => ({ ...b, logo: e.target.value || undefined }))
                        }
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setDraftBranding((b) => ({ ...b, favicon: e.target.value || undefined }))
                        }
                      />
                    </div>
                  </div>
                      ...
                      <div key={field} className="space-y-1.5">
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) =>
                              setDraftBranding((b) => ({ ...b, [field]: e.target.value }))
                            }
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setDraftBranding((b) => ({ ...b, headerText: e.target.value || undefined }))
                      }
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setDraftBranding((b) => ({ ...b, footerText: e.target.value || undefined }))
                      }
                    />
                  </div>
                </div>
                  ...
                  {draftDomain && (
                    <div className="rounded-lg border border-border bg-muted/40 p-4 space-y-3">
                      ...
                      <p className="text-xs text-muted-foreground flex items-start gap-1.5">
                        ...
                        DNS propagation can take up to 48 hours. SSL is provisioned automatically
                        once the CNAME is active.
                      </p>
                    </div>
                  )}
                  ...
                  {!draftDomain && (
                    <div className="rounded-lg border border-dashed border-border p-6 text-center space-y-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Enter a custom domain above to see DNS setup instructions.
                      </p>
                      <p className="text-xs text-muted-foreground/60">
                        Your portal URL is always available at{" "}
                        <code className="font-mono">/docs/{portal?.slug ?? "your-slug"}</code>
                      </p>
                    </div>
                  )}
                </div>
