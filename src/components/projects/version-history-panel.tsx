        ...
        {(version.meta?.commitSha || (version.meta?.agentsRun?.length ?? 0) > 0) && (
          <div className="px-5 py-2 border-b border-border/30 flex items-center gap-4 text-[11px] text-muted-foreground">
            ...
            {(version.meta?.changedFiles?.length ?? 0) > 0 && (
              <span>
                {version.meta.changedFiles!.length} file
                {version.meta.changedFiles!.length !== 1 ? "s" : ""} changed
              </span>
            )}
          </div>
        )}
...
export function VersionHistoryPanel({
  ...
  const [previewVersion, setPreviewVersion] = useState<(DocVersion & { content: string }) | null>(
    null,
  );
          ...
          {!loading && !error && versions.length === 0 && (
            <div className="flex flex-col items-center justify-center py-14 text-center px-5 gap-2">
              ...
              <p className="text-[12px] text-muted-foreground/60 leading-relaxed">
                Snapshots are saved automatically on each pipeline run and every time you save an
                edit.
              </p>
            </div>
          )}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {(v.meta?.changedFiles?.length ?? 0) > 0 && (
                              <p className="text-[11px] text-muted-foreground/70 mt-0.5">
                                {v.meta.changedFiles!.length} file
                                {v.meta.changedFiles!.length !== 1 ? "s" : ""} changed
                              </p>
                            )}
                          </div>
                        ...
                        {isConfirming && (
                          <div className="mt-2.5 rounded-md border border-border bg-muted/30 p-2.5 space-y-2">
                            <p className="text-[12px] text-foreground/80 leading-relaxed">
                              Replace the current content with this snapshot? The current state will
                              be saved first so you can undo.
                            </p>
                            <div className="flex gap-1.5">
                              <Button
                                size="sm"
